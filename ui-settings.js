...
function t(key){
  const fa = {
    ...
    desc2v2: 'شاهت مات شد؟ بازی برات تموم نمی‌شه! تخته ۱۶×۸ · حالت Guardian',
    ...
    resign: 'تسلیم', resignSeat: 'تسلیم (واگذاری به AI)', resignConfirm: 'تسلیم می‌شوی؟', endGameNoResult: 'پایان بدون نتیجه', rating2v2Title: 'ریتینگ ۲v۲', rating1v1Title: 'ریتینگ ۱v۱',
  ...
  const en = {
    ...
    desc2v2: 'Your king gets checkmated? The game isn\'t over for you! 16×8 board · Guardian mode',
    ...
    resign: 'Resign', resignSeat: 'Resign (hand to AI)', resignConfirm: 'Resign this game?', endGameNoResult: 'End — no result', rating2v2Title: '2v2 rating', rating1v1Title: '1v1 rating',
...
// 2v2 rating: a plain Elo update against the rating of the bot(s) faced.
// actualScore: 1 = win, 0.5 = draw, 0 = loss. oppRating: the bot's rating.
// Only called for a well-defined "human team vs all-AI team" 2v2 match —
// see shouldRate2v2Match() in game-4p.js for when that applies.
function accountBumpGame2v2(actualScore, oppRating){
  account = loadAccount();
  const current = (typeof account.rating2v2 === 'number') ? account.rating2v2 : 1000;
  const K = 32;
  const expected = 1 / (1 + Math.pow(10, (oppRating - current) / 400));
  account.rating2v2 = Math.round(current + K * (actualScore - expected));
  account.games2v2 = (account.games2v2 || 0) + 1;
  if (actualScore === 1) account.wins2v2 = (account.wins2v2 || 0) + 1;
  saveAccount(account);
  renderAccountCard();
...
function renderProfileView(){
  ...
  // Primary block: 2v2 stats.
  const games2v2 = account.games2v2 || 0;
  const wins2v2 = account.wins2v2 || 0;
  gamesEl.textContent = String(games2v2);
  winsEl.textContent = String(wins2v2);
  ratingEl.textContent = (typeof account.rating2v2 === 'number') ? String(account.rating2v2) : '—';
  wrEl.textContent = games2v2 > 0 ? (Math.round(wins2v2 / games2v2 * 100) + '%') : '—';

  // Secondary block: 1v1 stats (revealed by tapping the profile hero).
  const games1v1El = document.getElementById('profileView1v1Games');
  const wins1v1El = document.getElementById('profileView1v1Wins');
  const rating1v1El = document.getElementById('profileView1v1Rating');
  const wr1v1El = document.getElementById('profileView1v1WinRate');
  const games1v1 = account.games || 0;
  const wins1v1 = account.wins || 0;
  let rating1v1 = account.rating;
  const recent = (typeof c1LoadRecent === 'function') ? c1LoadRecent() : [];
  if (rating1v1 == null || rating1v1 === '') {
    const ratings = recent.map(g => parseInt(g.ratingEst, 10)).filter(n => !isNaN(n) && n > 0);
    if (ratings.length) rating1v1 = Math.round(ratings.reduce((a,b)=>a+b,0) / ratings.length);
  }
  if (games1v1El) games1v1El.textContent = String(games1v1);
  if (wins1v1El) wins1v1El.textContent = String(wins1v1);
  if (rating1v1El) rating1v1El.textContent = (rating1v1 != null && rating1v1 !== '') ? String(rating1v1) : '—';
  if (wr1v1El) wr1v1El.textContent = games1v1 > 0 ? (Math.round(wins1v1 / games1v1 * 100) + '%') : '—';

  const heroTap = document.getElementById('profileHeroTap');
  const block1v1 = document.getElementById('profileStats1v1Block');
  if (heroTap && block1v1 && !heroTap.dataset.wired) {
    heroTap.dataset.wired = '1';
    heroTap.addEventListener('click', () => {
      block1v1.style.display = (block1v1.style.display === 'none' || !block1v1.style.display) ? 'block' : 'none';
    });
  }
  // avatar
