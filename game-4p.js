...
// 2v2 bot personalities — same idea as the 1v1 C1_BOTS roster, mapped onto
// the existing 1-5 search-depth levels so no engine change is needed.
// `rating` is used both for display and as the opponent rating in the 2v2
// Elo update (see accountBumpGame2v2 in ui-settings.js).
const C4_BOTS = [
  { id: 'sina', name: 'سینا', nameEn: 'Sina', avatar: '🐤', rating: 300, depth: 1,
    style: 'تازه‌کار؛ زیاد اشتباه می‌کند', styleEn: 'Beginner — blunders often', tag: 'مبتدی', tagEn: 'Beginner' },
  { id: 'roya', name: 'رویا', nameEn: 'Roya', avatar: '🌙', rating: 750, depth: 2,
    style: 'صبور و محافظه‌کار', styleEn: 'Patient and cautious', tag: 'آسان', tagEn: 'Easy' },
  { id: 'daryush', name: 'داریوش', nameEn: 'Daryush', avatar: '⚔️', rating: 1300, depth: 3,
    style: 'برای هم‌تیمی‌اش می‌جنگد', styleEn: 'Fights hard for their teammate', tag: 'متوسط', tagEn: 'Intermediate' },
  { id: 'shirin', name: 'شیرین', nameEn: 'Shirin', avatar: '🧠', rating: 1700, depth: 4,
    style: 'تاکتیکی؛ دنبال تلهٔ دوتایی', styleEn: 'Tactical — hunts double threats', tag: 'سخت', tagEn: 'Hard' },
  { id: 'babak', name: 'ژنرال بابک', nameEn: 'General Babak', avatar: '👑', rating: 2100, depth: 5,
    style: 'قوی‌ترین فرمانده ۲v۲', styleEn: 'Strongest 2v2 commander', tag: 'استاد', tagEn: 'Master' },
];
function c4BotByDepth(depth) {
  return C4_BOTS.find(b => b.depth === depth) || null;
}
...
const el = {
  ...
  resignBtn: document.getElementById('resign4pBtn'),
  endGameBtn: document.getElementById('endGame4pBtn'),
...
function render() {
  ...
  const anyHumanSeat = PLAYERS.some(p => PLAYER_MODE[p] === 'human');
  // Resign only makes sense if there's a human seat to hand over.
  if (el.resignBtn) el.resignBtn.style.display = (anyHumanSeat && !state.gameOver) ? '' : 'none';
  // Ending without a result is meaningful any time a game is running,
  // including an all-AI spectate match someone just wants to stop early.
  if (el.endGameBtn) el.endGameBtn.style.display = !state.gameOver ? '' : 'none';
...
function openRematchSetup() {
  // Pre-fill the setup screen with the settings from the match that just
  // ended, so "rematch" opens a small, already-filled-in menu the person
  // can tweak (or just re-confirm) instead of silently reusing old settings.
  for (const p of PLAYERS) {
    const seg = document.querySelector('.setup-player-row .segmented[data-player="' + p + '"]');
    if (!seg) continue;
    seg.querySelectorAll('.seg-btn').forEach(btn => {
      btn.classList.toggle('active', btn.dataset.value === PLAYER_MODE[p]);
    });
  }
  const diffSel = document.getElementById('setupAiDifficulty');
  if (diffSel && el.aiDifficulty) diffSel.value = el.aiDifficulty.value;
  document.querySelectorAll('#timeControlPicker .seg-btn').forEach(btn => {
    btn.classList.toggle('active', parseInt(btn.dataset.minutes, 10) === selectedTimeControlMinutes);
  });
  showView('botSetupView');
}

// Only rate a clean human-team-vs-all-AI-team match with a real result.
// Mixed human/AI on both sides, all-human, all-AI, or a voided game are
// all left out since there's no well-defined single opponent rating.
function applyRatingUpdate2v2() {
  try {
    if (!state || !state.gameOver || state.voided) return;
    if (typeof accountBumpGame2v2 !== 'function') return;

    const humanTeams = new Set(PLAYERS.filter(p => PLAYER_MODE[p] === 'human').map(p => TEAM_OF[p]));
    if (humanTeams.size !== 1) return; // no human, or humans on both teams
    const humanTeam = [...humanTeams][0];
    const oppTeam = humanTeam === 'blue' ? 'red' : 'blue';
    const oppAllAI = PLAYERS.filter(p => TEAM_OF[p] === oppTeam).every(p => PLAYER_MODE[p] === 'ai');
    if (!oppAllAI) return;

    const bot = c4BotByDepth(currentAIDepth());
    if (!bot) return;

    const actual = !state.winnerTeam ? 0.5 : (state.winnerTeam === humanTeam ? 1 : 0);
    accountBumpGame2v2(actual, bot.rating);
  } catch (_) {}
}
...
if (el.rematchBtn) el.rematchBtn.addEventListener('click', openRematchSetup);
...
(function wireNewHomeCards(){
  ...
  if (cont) cont.addEventListener('click', () => {
    const fmt = document.querySelector('#gmFormatPicker .seg-btn.active')?.dataset.format || '2v2';
    window._gmSelected = { format: fmt };
    if (fmt === '1v1') {
      c1InitStockfish();
...
// --- End Game (void — no winner/loser) ------------------------------------
function endGame4pCurrent() {
  if (!state || state.gameOver) return;
  const en = (typeof appSettings !== 'undefined' && appSettings.lang === 'en');
  const ok = window.confirm(en
    ? 'The game will end without a result (no winner or loser). Continue?'
    : 'بازی بدون نتیجه (بدون برنده یا بازنده) تمام می‌شود. ادامه می‌دهید؟');
  if (!ok) return;
  stopPendingAI();
  stopTurnTimer();
  stopMatchClock();
...
if (el.endGameBtn) el.endGameBtn.addEventListener('click', endGame4pCurrent);

// --- Seat orientation: keep the human-controlled side toward the bottom ---
// The board itself doesn't rotate - only which team panel (blue/red) is
// drawn above vs below it, so the human's own pieces/badges sit closer to
// the bottom of the screen. Left alone (default: blue on top, red on
// bottom) when both or neither team has a human, since there's no clear
// preference in that case.
function orientBoardForHumans() {
  try {
    const centerCol = document.querySelector('.center-col');
    const bluePanel = document.querySelector('.center-col > .team-panel.blue');
    const redPanel = document.querySelector('.center-col > .team-panel.red');
    if (!centerCol || !bluePanel || !redPanel) return;
    const blueHasHuman = PLAYER_MODE.A === 'human' || PLAYER_MODE.B === 'human';
    const redHasHuman = PLAYER_MODE.C === 'human' || PLAYER_MODE.D === 'human';
    if (blueHasHuman && !redHasHuman) {
      centerCol.insertBefore(redPanel, centerCol.firstChild);
      centerCol.appendChild(bluePanel);
    } else {
      centerCol.insertBefore(bluePanel, centerCol.firstChild);
      centerCol.appendChild(redPanel);
    }
  } catch (_) {}
}
